...
const EachPartInput: React.FC<EachInput> = (props) => {
  const [openModal, setOpenModal] = useState<boolean>(false);
  const [image, setImage] = useState<File>();

  // 입력 값 업데이트 함수
  const updateInput = (key: "image" | "description", value: any) => {
  ...
  const handleUploadImage = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (props.input.departmentId) {
        await putDepartment(props.input.departmentId, props.input.description, image ? image : undefined);
        console.log("부서 업데이트 성공");
      } else {
        const response = await postDepartment(props.input.description, image ? image : undefined);
  ...
  const handleConfirmRemove = () => {
    ...
    updateInput("description", ""); // 캡션 초기화
        ...
        <S.TopDiv>
          ...
          <S.AddContentDiv>
            ...
            <S.Input
              ...
              onChange={(e) => updateInput("description", e.target.value)} // 캡션 수정 시 상태 업데이트
